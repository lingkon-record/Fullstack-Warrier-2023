        ...
        <div className="grid lg:grid-cols-5 md:grid-cols-3 grid-cols-1 gap-6">
          {categories.map((item, index) => (
            <div
              ...
              key={index}
              className="flex flex-col items-center cursor-pointer p-4 bg-gray-100 dark:bg-gray-700 rounded-lg hover:shadow-xl transition-shadow duration-300 ease-in-out"
            >
              <div className="mb-4">
                <img src={item.img} alt={item.item} className="w-28 h-28 object-cover rounded-full shadow-md" />
              </div>
              <div className="text-center">
                <h6 className="font-medium text-lg text-gray-900 dark:text-white mb-1">
                  {item.item}
                </h6>
