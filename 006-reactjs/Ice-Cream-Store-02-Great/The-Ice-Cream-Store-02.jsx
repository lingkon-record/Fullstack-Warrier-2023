...
const IceCreamOrder = () => {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
    {iceCreams.map((iceCream, index) => (
      <div key={index} className="relative flex flex-col bg-gray-800 rounded-2xl shadow-lg overflow-hidden transform transition-transform hover:scale-105 hover:shadow-2xl duration-500">
        <div className="relative">
          ...
          <h2 className="absolute bottom-4 left-4 text-3xl font-extrabold">{iceCream.name}</h2>
        </div>
        <div className="p-6 flex-1 flex flex-col justify-between">
          <p className="text-gray-300 mb-4">{iceCream.description}</p>
          <div className="flex items-center justify-between">
            <span className="text-xl font-bold text-yellow-400">${iceCream.price}</span>
            <button className="py-2 px-4 bg-yellow-500 text-gray-800 rounded-full hover:bg-yellow-600 transition-colors">Order Now</button>
          </div>
        </div>
      </div>
    ))}
